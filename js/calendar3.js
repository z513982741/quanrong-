...
Calendar.prototype.draw = function() {
    ...
    this.form.clearButton.onclick = function() {
        ...
        if( typeof afterDataChanges === 'function' ){
            afterDataChanges(calendar.dateControl,"")
        }
        if (calendar
    ...
    this.form.selectTodayButton.onclick = function() {
        ...
        calendar.hide();
        if( typeof afterDataChanges === 'function' ){
            afterDataChanges(calendar.dateControl,calendar.dateControl.value)
        }
    }
};
        ...
        if (dateArray[i]) {
            tds[i].onclick = function() {
                if (calendar.dateControl) {
                    ...
                    //if (calendar.getElementById(calendar.dateControl.attributes["id"].nodeValue + "_show")) {
                    //    calendar.getElementById(calendar.dateControl.attributes["id"].nodeValue + "_show").innerHTML = calendar.dateControl.value;
                    //}
                }
                calendar.hide();
                if( typeof afterDataChanges === 'function' ){
...
__ci.document
    .writeln('body {font-family:Helvetica,Arial,Tahoma;font-size:12px;margin:0px;text-align:center;}');
...
//点击时间input框，不选时间点击其他位置，隐藏控件
function calendarclose(){
	document.body.onclick=function(){
		obj=document.elementFromPoint(event.clientX,event.clientY);
		var id = obj.id;
		if(!Tools.check.isEmpty(obj.tagName)&&id!="stime"&&id!="etime"&&id!="vedat"&&id!="birthdayStart"&&id!="birthdayEnd"){
			calendar.hide();
		}
	}
